<script setup>
useHead({ title: 'Notification Details - Agent Panel' })
...
const loadNotification = async () => {
    ...
    error.value = null
    try {
        const response = await $fetchCitizen(`agent/v1/leads/notifications/${route.params.id}`, {
            method: 'GET'
        })
...
const markAsRead = async () => {
    try {
        ...
        await $fetchCitizen(`agent/v1/leads/notifications/${route.params.id}/read`, {
            method: 'POST',
            body: formData
        })
    ...
    <div class="w-full max-w-[1316px] mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div v-else-if="notification" class="bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <div class="p-6">
                ...
                <div v-if="notification.data?.creator_name" class="mt-6 space-y-2">
                    <div class="flex items-center gap-2 text-sm">
                        <span class="text-gray-500">From:</span>
                        <span class="text-gray-700 font-medium">{{ notification.data.creator_name }}</span>
                    </div>
                </div>
            </div>
