...
const updateHandler = async () => {
    ...
    try {
        ...
        const getData = await $fetchAdmin(`admin/roles/${props.item.id}`, {
            method: 'PUT',
            body: formData.value,
        });
...
const createHandler = async () => {
    ...
    try {
        isLoading.value = true;
        const getData = await $fetchAdmin(`admin/roles`, {
            method: 'POST',
            body: formData.value,
        });
        response_modal.value = getData;
    ...
    <Dialog v-model:visible="visible" modal :closable="false" :style="{ width: '25rem' }"
        ...
        <div class="flex items-center gap-4 mb-8">
            ...
            <div class="flex-auto">
                ...
                <InputError class="text-sm mt-1" :message="validations_errors.status" :text_size="'text-sm'" />
            </div>
        </div>
