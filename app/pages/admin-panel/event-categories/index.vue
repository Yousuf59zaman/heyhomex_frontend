...
const deleteHandler = async () => {
    ...
    try {
        const getData = await $fetchAdmin(`admin/event-categories/${deleteId.value}`, {
            method: 'DELETE',
        });
...
const restoreHandler = async (id) => {
    response_modal.value = {};
    try {
        const getData = await $fetchAdmin(`admin/event-categories/restore/${id}`, {
            method: 'POST',
        });
        if (getData.status == true) {
            response_modal.value = getData;
            ...
            <div class="pb-2 flex flex-col justify-between w-full">
                <div class="mt-4 border border-gray-200 rounded-lg bg-white dark:bg-gray-800">
                    <div class="border-b border-gray-200">
                        <h4 class="text-lg font-semibold text-gray-800 dark:text-gray-200 py-2 px-4">Event Categories
                        </h4>
                    </div>
                    <div class="p-4">
                            ...
                            <table class="table table-auto">
                                ...
                                <tbody v-else class="">
                                    <tr v-for="(item, index) in data" :key="index">
                                        ...
                                        <td class="text-gray-800 dark:text-gray-200">{{ item.parent ?
                                            item.parent.title : 'None' }}</td>
