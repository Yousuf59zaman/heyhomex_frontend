...
const loadData = async () => {
    ...
    permissions.value = {};
    try {
        const getData = await $fetchAdmin(`admin/portfolios/all`, {
            method: 'POST',
            body: {
                paginate: true,
...
const loadPortfolioCategories = async () => {
    try {
        const getData = await $fetchAdmin(`admin/portfolio-categories`, {
            method: 'GET',
        });
...
const deleteHandler = async () => {
    ...
    try {
        const getData = await $fetchAdmin(`admin/portfolios/${deleteId.value}`, {
            method: 'DELETE',
        });
...
const restoreHandler = async (id) => {
    response_modal.value = {};
    try {
        const getData = await $fetchAdmin(`admin/portfolios/restore/${id}`, {
            method: 'POST',
        });
        if (getData.status == true) {
            response_modal.value = getData;
