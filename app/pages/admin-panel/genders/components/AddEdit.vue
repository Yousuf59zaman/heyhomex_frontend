...
const updateHandler = async () => {
    ...
    try {
        ...
        const getData = await $fetchAdmin(`admin/genders/${props.item.id}`, {
            method: 'PUT',
            body: formData.value,
        });
...
const createHandler = async () => {
    ...
    try {
        isLoading.value = true;
        const getData = await $fetchAdmin(`admin/genders`, {
            method: 'POST',
            body: formData.value,
        });
        response_modal.value = getData;
    ...
    <Dialog v-model:visible="visible" modal :closable="false" :style="{ width: '30rem' }"
        ...
        <template #header>
            <div class="flex items-center justify-center w-full gap-2">
                <h4 class="text-xl font-semibold">{{ modalTitle }} Gender</h4>
            </div>
        </template>
        <div class="grid grid-cols-1 gap-4">
            <div class="flex items-center gap-4">
                ...
                <div class="flex-auto">
                    <LazyInputText v-model="formData.gender_name" class="w-full" placeholder="i.e. Male"
                        :class="validations_errors.gender_name ? 'border-[#f44336!important]' : ''" autocomplete="off"
                        @focus="validations_errors.gender_name = ''" />
                    <LazyInputError class="text-sm mt-1" :message="validations_errors.gender_name" />
                </div>
            </div>
