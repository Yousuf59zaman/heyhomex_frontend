...
const { $typetIconList } = useNuxtApp();
const iconList = $typetIconList();
...
watch(() => props.item, (value) => {
    if (value && Object.keys(value).length > 0) {
        ...
        formData.value = {
            ...
            url: value.url || '',
            hierarchy: String(value.hierarchy || ''),
...
const updateHandler = async () => {
    ...
    try {
        ...
        const getData = await $fetchAdmin(`admin/social-links/${props.item.id}`, {
            method: 'PUT',
            body: formData.value,
        });
...
const createHandler = async () => {
    ...
    try {
        isLoading.value = true;
        const getData = await $fetchAdmin(`admin/social-links`, {
            method: 'POST',
            body: formData.value,
        });
        response_modal.value = getData;
    ...
    <Dialog v-model:visible="visible" modal :closable="false" :style="{ width: '40rem' }"
        ...
        <template #header>
            <div class="flex items-center justify-center w-full gap-2">
                <h4 class="text-xl font-semibold">{{ modalTitle }} Social Links</h4>
            </div>
        </template>
        <div class="grid grid-cols-1 gap-4">
            ...
            <div class="flex items-center gap-4">
                <label class="font-semibold min-w-28">Display</label>
                <div class="flex-auto">
