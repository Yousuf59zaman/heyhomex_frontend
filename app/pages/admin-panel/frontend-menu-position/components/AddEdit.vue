...
const updateHandler = async () => {
    ...
    try {
        ...
        const getData = await $fetchAdmin(`admin/positions/${props.item.id}`, {
            method: 'PUT',
            body: formData.value,
        });
...
const createHandler = async () => {
    ...
    try {
        isLoading.value = true;
        const getData = await $fetchAdmin(`admin/positions`, {
            method: 'POST',
            body: formData.value,
        });
        response_modal.value = getData;
    ...
    <Dialog v-model:visible="visible" modal :closable="false" :style="{ width: '30rem' }"
        ...
        <template #header>
            <div class="flex items-center justify-center w-full gap-2">
                <h4 class="text-xl font-semibold">{{ modalTitle }} Position</h4>
            </div>
        </template>
        <div class="grid grid-cols-1 gap-4">
            ...
            <div class="flex items-center gap-4">
                <label class="font-semibold w-24">Serials</label>
                <div class="flex-auto">
