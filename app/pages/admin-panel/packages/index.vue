...
const deleteHandler = async () => {
    ...
    try {
        ...
        const getData = await $fetchAdmin(`admin/packages/${deleteId.value}/delete`, {
            method: 'POST',
            body: formData,
        });
...
const restoreHandler = async (id) => {
    response_modal.value = {};
    try {
        const getData = await $fetchAdmin(`admin/packages/${id}/restore`, {
            method: 'POST',
        });
        if (getData.status == 'success') {
            response_modal.value = getData;
    ...
    <div class="h-full mx-auto max-w-7xl px-2 sm:px-6 lg:px-8 my-6">
        <div class="h-full w-full overflow-auto">
            <div class="w-full flex flex-wrap md:flex-nowrap justify-between items-center gap-4 mb-4">
                ...
                <!-- <Button v-else-if="permissions?.add" label="Create Package" @click="addNew" class="text-xs" /> -->
                <Button  label="Create Package" @click="addNew" class="text-xs" />
            </div>
            ...
            <div class="pb-2 flex flex-col justify-between w-full">
                <div class="mt-4 border border-gray-200 rounded-lg bg-white dark:bg-gray-800">
                    <div class="border-b border-gray-200">
                        <h4 class="text-lg font-semibold text-gray-800 dark:text-gray-200 py-2 px-4">Packages</h4>
                    </div>
                    <div class="p-4">
                            ...
                            <table class="table table-auto">
                                ...
                                <tbody v-else class="">
                                    <tr v-for="(item, index) in data" :key="index">
                                        ...
                                        <td class="text-gray-800 dark:text-gray-200 text-center">
                                            <span class="capitalize">{{ item.stripe_product_type }}</span>
                                        </td>
                                        <td class="text-gray-800 dark:text-gray-200 text-center">{{ item.duration_in_days }}
                                        </td>
                                        <td class="text-gray-800 dark:text-gray-200 text-center">${{ item.price }}
                                        </td>
                                        <td>
                                            <div class="flex justify-center items-center">
                                                <span v-if="item.status == 1 || item.status == true" class="text-green-600"><i
