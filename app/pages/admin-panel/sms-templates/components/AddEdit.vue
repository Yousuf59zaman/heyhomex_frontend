...
const updateHandler = async () => {
    ...
    try {
        ...
        const getData = await $fetchAdmin(`admin/sms-templates/${props.item.id}`, {
            method: 'PUT',
            body: formData.value,
        });
...
const createHandler = async () => {
    ...
    try {
        isLoading.value = true;
        const getData = await $fetchAdmin(`admin/sms-templates`, {
            method: 'POST',
            body: formData.value,
        });
        response_modal.value = getData;
    ...
    <Dialog v-model:visible="visible" modal :closable="false" :style="{ width: '40rem' }"
        ...
        <template #header>
            <div class="flex items-center justify-center w-full gap-2">
                <h4 class="text-xl font-semibold">{{ modalTitle }} Template</h4>
            </div>
        </template>
        <div class="grid grid-cols-1 gap-4">
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="flex-auto">
                    ...
                    <LazyInputError class="text-sm mt-1" :message="validations_errors.sms_body" />
                </div>
            </div>
