...
const loadData = async () => {
    ...
    permissions.value = {};
    try {
        const getData = await $fetchAdmin(`admin/events/all`, {
            method: 'POST',
            body: {
                paginate: true,
...
const deleteHandler = async () => {
    ...
    try {
        const getData = await $fetchAdmin(`admin/events/${deleteId.value}`, {
            method: 'DELETE',
        });
...
const restoreHandler = async (id) => {
    response_modal.value = {};
    try {
        const getData = await $fetchAdmin(`admin/events/restore/${id}`, {
            method: 'POST',
        });
        if (getData.status == true) {
            response_modal.value = getData;
    ...
    <div class="h-full mx-auto max-w-7xl px-2 sm:px-6 lg:px-8 my-6">
        <div class="h-full w-full overflow-auto">
            <div class="w-full flex flex-wrap md:flex-nowrap justify-between items-center gap-4 mb-4">
                ...
                <Button v-else-if="permissions?.add" label="Create Event" @click="addNew" class="text-xs" />
            </div>
            ...
            <div class="pb-2 flex flex-col justify-between w-full">
                <div class="mt-4 border border-gray-200 rounded-lg bg-white dark:bg-gray-800">
                    <div class="border-b border-gray-200">
                        <h4 class="text-lg font-semibold text-gray-800 dark:text-gray-200 py-2 px-4">Events</h4>
                    </div>
                    <div class="p-4">
                            ...
                            <table class="table table-auto">
                                <thead class="sticky z-10 top-0">
                                    <tr>
                                        ...
                                        <th width="5%" v-if="(permissions.edit || permissions.delete) || isLoading">
                                            <div
                                                class="flex flex-row items-center justify-center gap-2 text-gray-800 dark:text-gray-200">
                                                <span>Action</span>
                                            </div>
