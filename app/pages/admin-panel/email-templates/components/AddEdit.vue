...
const updateHandler = async () => {
    ...
    try {
        ...
        const getData = await $fetchAdmin(`admin/email-templates/${props.item.id}`, {
            method: 'PUT',
            body: formData.value,
        });
...
const createHandler = async () => {
    ...
    try {
        isLoading.value = true;
        const getData = await $fetchAdmin(`admin/email-templates`, {
            method: 'POST',
            body: formData.value,
        });
        response_modal.value = getData;
...
<template>
    <Dialog v-model:visible="visible" modal :closable="false" :style="{ width: '40rem' }"
        ...
        <div class="grid grid-cols-1 gap-4">
            ...
            <div class="flex items-center gap-4">
                <label class="font-semibold w-28">Email Body</label>
                <div class="flex-auto">
