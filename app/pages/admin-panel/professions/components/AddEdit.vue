...
const updateHandler = async () => {
    ...
    try {
        ...
        const updateData = await $fetchAdmin(`admin/professions/${props.item.id}/update`, {
            method: 'POST',
            body: {
                ...formData.value,
                _method: 'PATCH'
...
const createHandler = async () => {
    ...
    try {
        isLoading.value = true;
        const getData = await $fetchAdmin(`admin/professions/store`, {
            method: 'POST',
            body: formData.value,
        });
        response_modal.value = {
    ...
    <Dialog v-model:visible="visible" modal :closable="false" :style="{ width: '30rem' }"
        ...
        <template #header>
            <div class="flex items-center justify-center w-full gap-2">
                <h4 class="text-xl font-semibold">{{ modalTitle }} Profession</h4>
            </div>
        </template>
        <div class="grid grid-cols-1 gap-4">
            <div class="flex items-center gap-4">
                ...
                <div class="flex-auto">
                    ...
                    <InputText v-model="formData.name" class="w-full" placeholder="i.e. Doctor"
