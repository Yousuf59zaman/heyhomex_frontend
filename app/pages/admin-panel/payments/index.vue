...
const loadData = async () => {
    ...
    permissions.value = {};
    try {
        const getData = await $fetchAdmin(`admin/payments/list`, {
            method: 'GET',
            params: {
                    ...
                    <div class="flex flex-wrap md:flex-nowrap items-center gap-4 w-full md:w-auto">
                        <div class="flex items-center gap-2 w-full md:w-auto">
                            ...
                            <InputText type="text" v-model="search" @input="loadData" @keyup.enter="loadData"
                                class="w-full md:w-auto" placeholder="Search by user or invoice..." />
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="pb-2 flex flex-col justify-between w-full">
                <div class="mt-4 border border-gray-200 rounded-lg bg-white dark:bg-gray-800">
                    <div class="border-b border-gray-200">
                        <h4 class="text-lg font-semibold text-gray-800 dark:text-gray-200 py-2 px-4">Payments List</h4>
                    </div>
                    <div class="p-4">
                                    ...
                                    <tr v-for="(item, index) in data" :key="index">
                                        ...
                                        <td class="text-gray-800 dark:text-gray-200">
                                            <div class="flex flex-col">
                                                <span class="font-medium">User #{{ item.user_id }}</span>
                                                <span v-if="item.stripe_invoice_details?.customer_email" class="text-xs text-gray-500">
                                                    {{ item.stripe_invoice_details.customer_email }}
                                                </span>
                                            </div>
                                        </td>
                                        <td class="text-gray-800 dark:text-gray-200 text-center text-sm">
                                            {{ item.stripe_invoice_details?.invoice_number || 'N/A' }}
                                        </td>
                                        <td class="text-gray-800 dark:text-gray-200 text-center font-medium">
                                            ${{ item.amount }}
                                        </td>
                                        <td class="text-gray-800 dark:text-gray-200 text-center uppercase">
                                            {{ item.currency }}
                                        </td>
                                        ...
                                        <td class="text-gray-800 dark:text-gray-200 text-center text-sm">
                                            {{ formatDate(item.stripe_invoice_details?.created || item.created_at) }}
                                        </td>
                                        <td class="text-center">
                                            <div v-if="item.stripe_invoice_details" class="flex justify-center items-center gap-2">
                                                <a v-if="item.stripe_invoice_details.invoice_pdf" 
                                                   :href="item.stripe_invoice_details.invoice_pdf" 
                                                   target="_blank"
                                                   class="text-blue-600 hover:text-blue-800 transition"
                                                   title="Download PDF">
                                                    <i class="fa-solid fa-file-pdf"></i>
                                                </a>
                                                <a v-if="item.stripe_invoice_details.hosted_invoice_url" 
                                                   :href="item.stripe_invoice_details.hosted_invoice_url" 
                                                   target="_blank"
                                        ...
                                        <!-- <td class="text-gray-800 dark:text-gray-200 text-center text-xs">
                                            <span class="truncate inline-block max-w-[100px]" :title="item.stripe_invoice_id">
                                                {{ item.stripe_invoice_id || 'N/A' }}
                                            </span>
                                        </td> -->
                                    </tr>
                                </tbody>
