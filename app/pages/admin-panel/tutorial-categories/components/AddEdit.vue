...
// Filter out current item from parent options
const parentOptions = computed(() => {
    if (!props.item?.id || !props.data) return props.data || [];
    return props.data.filter(item => item.id !== props.item.id);
});
...
const updateHandler = async () => {
    ...
    try {
        ...
        const getData = await $fetchAdmin(`admin/tutorial-categories/${props.item.id}`, {
            method: 'PUT',
            body: formData.value,
        });
    ...
    <Dialog v-model:visible="visible" modal :closable="false" :style="{ width: '30rem' }"
        ...
        <div class="grid grid-cols-1 gap-4">
            <div class="flex items-center gap-4">
                ...
                <div class="flex-auto">
                    <LazyInputText v-model="formData.title" class="w-full" placeholder="i.e. Tutorials"
            ...
            <div class="flex items-center gap-4">
                ...
                <div class="flex-auto">
                    <LazyInputText v-model="formData.slug" class="w-full" placeholder="i.e. slug-tutorials"
