        ...
        <div class="flex items-start gap-3">
            <div class="relative flex-shrink-0">
                <img
                    :src="video.thumbnail"
                    :alt="video.title"
                    class="w-20 h-14 rounded-md object-cover" />
                
                <!-- Play Icon Overlay -->
                <div class="absolute inset-0 flex items-center justify-center bg-black/30 rounded-md">
                    <div class="bg-red-600 rounded-full p-1">
                        <Icon name="lucide:play" class="w-3 h-3 text-white" />
                    </div>
                </div>
                
                <!-- Duration Badge -->
                <div class="absolute bottom-1 right-1 bg-black/75 text-white text-xs px-1 py-0.5 rounded">
                    {{ video.duration }}
                </div>
            </div>
            
            <div class="flex-1 min-w-0">
                <h4 class="font-semibold text-gray-900 text-sm mb-1 line-clamp-2 leading-tight">
                    {{ video.title }}
                </h4>
                ...
                <div class="flex items-center justify-between">
                    ...
                    <div class="flex items-center text-xs text-gray-600">
                        <Icon name="lucide:map-pin" class="w-3 h-3 mr-1" />
                        {{ video.location }}
                    </div>
                </div>
            </div>
