...
const responseFormat = (response: any) => {
    if (response?.status && response?.data) {
        ...
        if (props.preselectedAccountType) {
            const userTypeIds = userData.user_type?.map((type: any) => type.id) || []
            const isValidForPanel = (() => {
                if (props.preselectedAccountType === 1) {

                    return userTypeIds.includes(4)
                } else if (props.preselectedAccountType === 2) {

                    return userTypeIds.includes(5)
                } else if (props.preselectedAccountType === 3) {

                    return userTypeIds.some((id: number) => [1, 2, 3].includes(id)) || userTypeIds.length === 0
                }
                return false
            })()

            if (!isValidForPanel) {
                const panelName = props.preselectedAccountType === 1 ? 'Advertiser' : props.preselectedAccountType === 2 ? 'Agent' : 'User'
                validations_errors.value = `You are not registered as ${panelName === 'User' ? 'a' : 'an'} ${panelName}. Please use the correct panel to login.`
                return
            }
        }
...
<template>
    <Dialog v-model:visible="visible" modal :closable="false" :draggable="false" :resizable="false"
        class="citizen-login-modal" :style="{ width: '45rem', maxWidth: '720px' }" :pt="{
        ...
        <template #header>
            ...
            <div class="w-full text-center px-4 sm:px-6 pt-8 md:pt-9 pb-1">
                <h1
                    class="text-[1.75rem] sm:text-3xl lg:text-[32px] leading-snug lg:leading-[40px] font-[510] text-[#121A22] font-['sf-pro-Medium']">
        ...
        <div class="px-6 pb-10 pt-6">
            <form @submit.prevent="handleLogin" class="flex flex-col gap-6" autocomplete="off">
                <div class="flex flex-col gap-2">
                    <label for="email"
                        class="text-base leading-6 font-[510] text-[#121A22] font-['sf-pro-Medium'] [font-feature-settings:'dlig'_on]">Email
                        Address</label>
                    <LazyInputText id="email" v-model="formData.email" type="email"
                        placeholder="Enter your email address" required autocomplete="off"
                        class="w-full px-4 h-14 border border-[#CFDBE8] rounded-lg text-base leading-6 font-normal text-[#121A22] font-['sf-pro-Regular'] [font-feature-settings:'dlig'_on] placeholder:text-[#566573] focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" />
                </div>

                <div class="flex flex-col gap-2">
                    <label for="password"
                        class="text-base leading-6 font-[510] text-[#121A22] font-['sf-pro-Medium'] [font-feature-settings:'dlig'_on]">Password</label>
                    <div class="relative">
                        <LazyInputText id="password" v-model="formData.password"
                            :type="password_open ? 'text' : 'password'" placeholder="Enter your password" required
                            autocomplete="current-password"
                            class="w-full px-4 h-14 border border-[#CFDBE8] rounded-lg text-base leading-6 font-normal text-[#121A22] font-['sf-pro-Regular'] [font-feature-settings:'dlig'_on] placeholder:text-[#566573] focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-colors" />
                        <button type="button" @click="password_view_status(!password_open)"
                            class="absolute right-4 top-1/2 transform -translate-y-1/2 text-gray-400 hover:text-gray-600"
                            :aria-pressed="password_open" aria-label="Toggle password visibility">
                            <i :class="[
                                'fa text-lg',
                ...
                <div v-if="validations_errors"
                    ...
                    <button type="button" @click="validations_errors = ''"
                        class="text-[#DC2626] hover:text-red-800 flex-shrink-0">
                        <i class="pi pi-times text-sm"></i>
                    </button>
                </div>

                <button type="submit" :disabled="loading" :loading="loading" loadingIcon="pi pi-spin pi-spinner"
                ...
                <div class="relative flex items-center justify-center">
                    ...
                    <div class="relative px-4 bg-white">
                        <span class="text-base text-gray-500">or continue with</span>
                    </div>
                </div>

                <div class="grid grid-cols-3 mb-3 gap-3">
                    <button @click="handleAppleLogin" type="button" :disabled="true"
                    ...
                    <button @click="handleFacebookLogin" type="button" :disabled="true"
                        ...
                        <svg v-if="!isFacebookLoadingSSO" class="w-5 h-5 text-[#1877F2]" viewBox="0 0 24 24"
                            fill="currentColor">
                            <path
                                d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
                        </svg>
                        <span class="text-sm ml-2">Facebook</span>
                        <span v-if="isFacebookLoadingSSO"
                            class="inline-block w-5 h-5 border-2 border-gray-300 border-t-gray-900 rounded-full animate-spin"></span>
                    </button>

                    <button @click="handleGoogleLogin" type="button" :disabled="loading || isGoogleLoadingSSO"
                        class="px-4 py-3 bg-white hover:bg-gray-50 text-gray-900 font-medium border border-gray-300 rounded-lg transition-colors duration-200 flex items-center justify-center disabled:opacity-50 disabled:cursor-not-allowed">
                        <svg v-if="!isGoogleLoadingSSO" class="w-5 h-5" viewBox="0 0 24 24">
                            <path fill="#4285F4"
                ...
                <div class="text-center">
                    <p
                        ...
                        New to heyhomex?
                        <button type="button" @click="showRegister"
