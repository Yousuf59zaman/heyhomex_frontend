...
<template>
    <Teleport to="body">
        <Transition name="modal-fade" @after-leave="showInfo = true">
            <div v-if="isOpen && currentVideo"
                class="fixed inset-0 z-50 flex items-center justify-center bg-black/95 backdrop-blur-sm"
                @click="handleBackdropClick">
                <div
                    class="relative w-full h-full flex flex-col lg:flex-row p-2 sm:p-4 md:p-5 lg:p-6 gap-2 sm:gap-3 md:gap-4 overflow-y-auto">
                    <!-- Main Player Area -->
                    <div class="w-full lg:flex-1 flex flex-col">
                        ...
                        <div class="flex-shrink-0 w-full">
                            <div class="w-full max-w-full sm:max-w-7xl mx-auto">
                                <div class="w-full aspect-video">
                                    <VideoHlsJsPlayer v-if="currentVideo" ref="playerRef" :video="currentVideo"
                                        :autoplay="true" :advertising="{ client: adConfig.client ?? '', ...adConfig }"
                        ...
                        <Transition name="slide-up">
                            <div v-if="showInfo"
                                ...
                                <div
                                    class="flex flex-wrap items-center gap-2 sm:gap-3 md:gap-4 text-xs sm:text-sm text-white/80 mb-2 sm:mb-3">
                                ...
                                <p v-if="currentVideo.description || currentVideo.subtitle"
                                    class="text-sm text-white/70 leading-relaxed">
                                    {{ currentVideo.description || currentVideo.subtitle }}
                                </p>

                                <div v-if="currentVideo.category || currentVideo.location"
                                    class="flex flex-wrap gap-2 mt-3">
                                    <span v-if="currentVideo.category"
                                        class="px-3 py-1 bg-white/20 rounded-full text-xs font-medium">
                                        {{ currentVideo.category }}
                                    </span>
                                    <span v-if="currentVideo.location"
                    ...
                    <div v-if="playlist.length > 0"
                        ...
                        <div
                            class="flex-1 overflow-y-auto space-y-1.5 sm:space-y-2 scrollbar-thin scrollbar-thumb-white/30 scrollbar-track-transparent">
                            <div v-for="(video, index) in playlist" :key="video.id" @click="playFromPlaylist(video)"
                                :class="[
                                ...
                                <div
                                    class="relative flex-shrink-0 w-24 h-16 sm:w-28 sm:h-18 md:w-32 md:h-20 rounded overflow-hidden">
                                    <img :src="video.thumbnail" :alt="video.title" class="w-full h-full object-cover" />
                                    <div
                                        class="absolute bottom-1 right-1 bg-black/70 text-white px-1.5 py-0.5 rounded text-xs">
                                        {{ video.duration }}
                                    </div>
                                    <div v-if="video.id === currentVideo.id"
                                ...
                                <div class="flex-1 min-w-0">
                                    ...
                                    <div
                                        class="flex items-center gap-1 sm:gap-2 text-white/50 text-[10px] sm:text-xs mt-0.5 sm:mt-1">
