...
export type AuthStep = 'initial' | 'motivation' | 'budget' | 'location' | 'success'
...
export const STEP_CONFIG = {
  ...
  budget: {
    title: "You bring the dream. We'll map the way. ✨", 
    subtitle: "What's your sweet spot, budget-wise?\nNo judgment — just vibes and homes that fit",
    stepLabel: "Step: 2/3",
    fields: ['budget'],
  ...
  success: {
    ...
    subtitle: "We've saved your preferences and we're ready to help you find your perfect home.",
    stepLabel: "",
    fields: [],
    apiEndpoint: null,
    validation: {}
...
export const STEP_OPTIONS = {
  ...
  motivation: [
    ...
    { label: "Here on orders — base life meets island life", value: 'military_orders' },
    { label: "I'm scouting the next big opportunity", value: 'investment_opportunity' }
  ],
  budget: [
    ...
    return new Promise<{success: boolean, data?: any, error?: string}>((resolve) => {
      setTimeout(() => {
        ...
        if (Math.random() > 0.1) { // 90% success rate for demo
          ...
          resolve({ success: false, error: 'Failed to save data. Please try again.' })
        }
      }, 1500)
    })
  }
...
const validationService = {
  ...
  validateStep(step: AuthStep, data: Partial<UserFormData>): Record<string, string> {
    const config = STEP_CONFIG[step]
    ...
    config.fields.forEach(field => {
      ...
      if (rules) {
        const error = this.validateField(field, data[field as keyof UserFormData], rules)
        if (error) {
          errors[field] = error
        }
      }
...
export const useCitizenModal = () => {
  ...
  const nextStep = async (formData?: Partial<UserFormData>) => {
    ...
    const errors = validationService.validateStep(modalState.step, currentStepData)
    ...
    try {
      const result = await apiService.saveStepData(modalState.step, currentStepData)
      
      if (!result.success) {
        modalState.errors = { general: result.error || 'Failed to save data' }
        modalState.loading = false
        return false
      }
      ...
      // Move to next step
      const steps: AuthStep[] = ['initial', 'motivation', 'budget', 'location', 'success']
  ...
  const currentStepOptions = computed(() => {
    ...
    return {
      ...
      preferredLocation: step === 'initial' ? STEP_OPTIONS.preferredLocation : [],
